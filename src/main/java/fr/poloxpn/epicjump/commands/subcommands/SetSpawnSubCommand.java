...
public class SetSpawnSubCommand extends SubCommand {
    ...
    public SetSpawnSubCommand(EpicJump plugin) {
        super("/epicjump setspawn <ParkourName>", "Set the spawn of a parkour.", "epicjump.admin","setspawn");
        this.plugin = plugin;
    }
    ...
    protected void onExePlayer(Player player, String... args) {
        ...
        if(!plugin.getCourseManager().courseExists(args[1])) {
            ...
            plugin.getCourseManager().setSpawn(args[1], player.getLocation());
            player.sendMessage(Message.PREFIX + Message.getMessage(Message.Messages.ADD_SPAWN).replaceAll("%course%", args[1]));
        }
    }
